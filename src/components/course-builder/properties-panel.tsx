    ...
    <div className="h-full flex flex-col border-l bg-muted/30">
      ...
      <ScrollArea className="flex-1">
        <div className="p-4">
          ...
          {itemType === "module" && (
            <ModuleProperties
              item={selectedItem as Module}
              onUpdate={onUpdate}
            />
          )}
          {itemType === "lesson" && (
            <LessonProperties
              item={selectedItem as Lesson}
              onUpdate={onUpdate}
            />
          )}
          {itemType === "slide" && (
            <SlideProperties item={selectedItem as Slide} onUpdate={onUpdate} />
          )}
          {itemType === "block" && (
            <BlockProperties
              item={selectedItem as ContentBlock}
              onUpdate={onUpdate}
            />
          )}
        </div>
  ...
  return (
    <Tabs defaultValue="general" className="w-full">
      ...
      <TabsContent value="settings" className="space-y-4">
        ...
        <div className="space-y-2">
          <Label htmlFor="publish-date">Scheduled Publish</Label>
          <Input
            id="publish-date"
            type="datetime-local"
            value={
              item.settings.publishDate
                ? new Date(item.settings.publishDate).toISOString().slice(0, 16)
                : ""
            }
            onChange={(e) =>
              onUpdate({
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        <Input
          ...
          onChange={(e) =>
            onUpdate({ ...item, xpReward: parseInt(e.target.value) || 10 })
          }
        />
      </div>
...
function BlockProperties({
  ...
  item: ContentBlock;
  onUpdate: (item: ContentBlock) => void;
}) {
  return (
    <div className="space-y-4">
      <Card>
